...
(function ($) {
	//modules which need show in top of list
	var contentLayoutModule = "Content Layout";
	var htmlModule = 'HTML Pro';
    ...
    dnnModuleDialog.prototype = {
        ...
        _loadModuleList: function (val, isSearch) {
            if ((this._loadMore || isSearch) && !this._getService().isLoading()) {
                if (isSearch && this._startIndex == 0) {
                    $('.listContainer ul li.dnnModuleItem').remove();
        ...
        _renderModuleList: function (data) {
            ...
            if (data.length > 0) {
	            ...
	            var contentLayoutModuleIndex = -1;
                for (var i = 0; i < data.length; i++) {
                    if (data[i].ModuleName === htmlModule) {
	                    htmlModuleIndex = i;
                    }
                    else if (data[i].ModuleName === contentLayoutModule) {
	                    contentLayoutModuleIndex = i;
                    }
                }
				if (htmlModuleIndex > -1) {
					data.splice(0, 0, data.splice(htmlModuleIndex, 1)[0]);
					contentLayoutModuleIndex++;
				}

				if (contentLayoutModuleIndex > -1) {
					data.splice(0, 0, data.splice(contentLayoutModuleIndex, 1)[0]);
				}
        ...
        _renderItem: function (item) {
            ...
            for (var a in item) {
				...
				if (item.ModuleName === htmlModule || item.ModuleName === contentLayoutModule) {
