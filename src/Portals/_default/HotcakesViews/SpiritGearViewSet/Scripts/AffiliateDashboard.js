...
var HcAffiliateProfileViewModel = function (model, $form, res) {
    var self = this;
    ...
    self.update = function () {
        ...
        .done(function (res) {
            self.showMessage(res.Message, res.Status);
        })
        .fail(function () { })
    ...
    self.changeReferralAffiliateId = function () {
        ...
        $.post(hcc.getServiceUrl("AffiliateRegistration/IsAffiliateValid"),
            ...
            .done(function (data) {
                if (!data) {
                    self.showMessage(res.ValMessage_ReferralAffiliateIDInvalid, "Failed");
                }
                else {
                    self.message.show(false);
                }
...
var HcBaseAffiliateDashboardReport = function (data, $form, res) {
    var self = this;
...
var HcBaseAffiliateDashboardDateReport = function (data, $form, res) {
    var self = this;

    self.dateRangesList = ko.observable(res.timeRangesLocalized);
    ...
    HcBaseAffiliateDashboardReport.call(self, data, $form, res);
};

var HcAffiliateOrders = function (data, $form, res) {
    var self = this;
    self.orders = ko.observableArray(data.Orders);
    self.loadPageData = function (resp) {
        self.orders.removeAll();
        $.each(resp.Orders, function (i, el) { self.orders.push(el); });
    ...
    HcBaseAffiliateDashboardDateReport.call(self, data, $form, res);
};

var HcAffiliatePayments = function (data, $form, res) {
    var self = this;
    self.payments = ko.observableArray(data.Payments);
    self.loadPageData = function (resp) {
        self.payments.removeAll();
        $.each(resp.Payments, function (i, el) { self.payments.push(el); });
        self.pager.total(resp.TotalCount);
        self.totalCount(resp.TotalCount);
        self.totalAmount(resp.TotalAmount);
    };
...
var HcAffiliateReferrals = function (data, $form, res) {
    var self = this;
    ...
    self.loadPageData = function (resp) {
        self.referrals.removeAll();
        $.each(resp.Referrals, function (i, el) { self.referrals.push(el); });
        self.pager.total(resp.TotalCount);
        self.totalCount(resp.TotalCount);
        self.totalAmount(resp.TotalAmount);
    };
...
var HcUrlBuilderViewModel = function (data, $form, res) {
    var self = this;
    ...
    self.generate = function () {
        ...
        $.post(hcc.getServiceUrl("AffiliateDashboard/GenerateUrl"),
            ...
            .done(function (res) { self.generatedUrl(res); })
            .fail(function () { })
            .always(function () { $form.ajaxLoader("stop"); });
    ...
    var getProducts = function () {
        $.post(hcc.getServiceUrl("AffiliateDashboard/GetProducts"),
            ...
            .done(function (res) {
                ...
                $.each(res, function (i, el) { self.products.push(el); });
            })
            .fail(function () { })
            .always(function () { $form.ajaxLoader("stop"); });
    };
